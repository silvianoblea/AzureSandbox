...
$command = ".\config.cmd --unattended --pool 'dm7' --url 'https://dev.azure.com/WinOpsDBA' --auth pat --token $($token) --agent $($hostname) --replace --runAsService --windowsLogonAccount 'devops\vm-admin-sa' --windowsLogonPassword '$($service_password)' --noRestart --acceptTheEula"
